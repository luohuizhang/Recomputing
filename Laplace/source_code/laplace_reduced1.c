	 ...
	 */ 
{
	...
	for(i=0;i<8;i++){
		index=0;
		int index_x=(i/(nproc_x_full/nproc_x_reduced))*(nodeedge/nproc_x_full*nproc_x_reduced)+1+i1;
		int index_y=(i%(nproc_y_full/nproc_y_reduced))*(nodeedge/nproc_y_full*nproc_y_reduced)+1+j1;      
	...
	printf ( "Reduced: Task %i took %6.3f seconds\n", rank, wtime );
	 ...
	 */
{
	...
	if(rank==1)
		printf("Reduced: iteration number is %d\n",it);
