	 ...
	 */ 
{
	...
	starts[0] = 0;
	starts[1] = 0;
	submatrix[0]=nodeedge/((int)( nblock1)/nproc_x)+1;
	submatrix[1]=nodeedge/((int)( nblock2)/nproc_y)+1;
...
double **reduced_buffer;
	...
	buffer_all = malloc((nodeedge/((int)( nblock1)/nproc_x)+1)*(nodeedge/((int)( nblock2)/nproc_y)+1)*sizeof(double));



	reduced_buffer = malloc((nodeedge/((int)( nblock1)/nproc_x)+1)*sizeof(*reduced_buffer));

	for(i=0;i<(nodeedge/((int)( nblock1)/nproc_x)+1);i++)
	{
		reduced_buffer[i] =  buffer_all+i*(nodeedge/((int)( nblock2)/nproc_y)+1);

	}

//      printf("%d,%d\n",nodeedge/((int)( nblock1)/nproc_x)+1,nodeedge/((int)( nblock2)/nproc_y)+1);  
      double buffer[nodeedge/((int)( nblock1)/nproc_x)+1][nodeedge/((int)( nblock2)/nproc_y)+1];
	int destination= rank %((int)( nblock2))/(nblock2/nproc_y)+rank /((int)( nblock2))/(nblock1/nproc_x)*nproc_y;

	printf("rank %d is mapped to %d\n",rank, destination);
        //MPI_Recv ( &reduced_buffer[0][0], 1 , matrix_reduced2full, destination, 1,MPI_COMM_WORLD,MPI_STATUS_IGNORE);

//	regen_linear( M,  reduced_buffer,  ((int)( nblock2)/nproc_x), ((int)( nblock1)/nproc_y),  rank);
	...
	if ( rank == 0 )
	{
		...
		fclose ( fp );
		printf ( "  Solution written to \"laplace_solution_full.txt\".\n" );
	}
	/*
	   Terminate MPI.
	 */
