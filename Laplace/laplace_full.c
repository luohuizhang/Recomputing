...
# define n1 192            /* matrix is nxn, excluding boundary values     */
# define n2 192            /* matrix is nxn, excluding boundary values     */
	 ...
	 */ 
{
	...
	double w;
	double wtime;
	int nproc_x=atoi(argv[1]);
	int nproc_y=atoi(argv[2]);
	int nproc_reduced=nproc_x*nproc_y;
	...
	double reduced_buffer[(nodeedge/((int)( nblock1)/nproc_x)+1)*(nodeedge/((int)( nblock2)/nproc_y)+1)];




	//      printf("%d,%d\n",nodeedge/((int)( nblock1)/nproc_x)+1,nodeedge/((int)( nblock2)/nproc_y)+1);  
	int destination= rank %((int)( nblock2))/(nblock2/nproc_y)+rank /((int)( nblock2))/(nblock1/nproc_x)*nproc_y;

	//printf("rank %d is mapped to %d\n",rank, destination);
	MPI_Recv (&reduced_buffer[0],(nodeedge/((int)( nblock1)/nproc_x)+1)*(nodeedge/((int)( nblock2)/nproc_y)+1),MPI_DOUBLE , destination, 1,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
int index=0;        
/*for(i=0;i<=nodeedge;i++)
                for(j=0;j<=nodeedge/2;j++){

                              printf("%d,%d,%f,%f\n",i+1,2*j+1,M[1+i][1+j*2],reduced_buffer[index++]);

                }
*/
	regen_linear( M,  reduced_buffer,  ((int)( nblock1)/nproc_x), ((int)( nblock2)/nproc_y),  rank);
	...
	if ( rank == 0 )
	{
		fp = fopen ( "laplace_solution_full.txt", "w" );
...
void regen_linear(double M[][nodeedge+2], double* reduced_buffer, int ratio_x, int ratio_y, int rank)
{
	...
	for(i=0;i<=nodeedge/ratio_x;i++)
		for(j=0;j<=nodeedge/ratio_y;j++){

//				printf("%d,%d,%f,%f\n",i+1,j+1,M[1+ratio_x*i][1+j*ratio_y],reduced_buffer[index]);
			M[1+ratio_x*i][1+j*ratio_y]=reduced_buffer[index++];
	...
	for(i=1;i<=nodeedge;i++)
		for(j=1;j<=nodeedge;j++)
		{
			...
			int x_low=(i-1)/ratio_x*ratio_x+1;
			int y_low=(j-1)/ratio_y*ratio_y+1;

			int x_high=x_low+ratio_x;
			if(x_high>nodeedge+1)
				x_high=nodeedge+1;
			int y_high=y_low+ratio_y;
			if(y_high>nodeedge+1)
				y_high=nodeedge+1;
...
//			if( M[i][j]==0)
			//	printf("%d,%d,%d,%d: %f,%f,%d,%d\n",x_low,x_high,y_low,y_high, ii,jj,i,j);
		}
