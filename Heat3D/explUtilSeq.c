...
void initValues(int nb_layers, double*** x0, int x_dim, int y_dim, int z_dim, double temp1_init, double temp2_init)
{
 ...
 /*FILE* file;	
   ...
   //Setup temp2_init inside 
   for(k=nb_layers;k<=z_dim-(nb_layers-1);k=k+4){
	 file=fopen("input_mesh","r");
     for(j=0;j<=y_dim-(nb_layers-1);j=j+4)
   	for(i=0;i<=x_dim-(nb_layers-1);i=i+4)
