	 ...
	 */ 
{
	...
	double wtime;
        ...
        int nproc_full=nproc_x_full*nproc_y_full;
        int nproc_reduced=nproc_x_reduced*nproc_y_reduced;
	MPI_Init ( &argc, &argv );
        int world_rank;
	...
	iterate ( w, M, result, rank, comm,comm1 );
        ...
        for(i=0;i<nproc_full/nproc_reduced;i++){
                index=0;
                int index_x=(i/(nproc_y_full/nproc_y_reduced))*(nodeedge/nproc_x_full*nproc_x_reduced)+1;
                int index_y=(i%(nproc_y_full/nproc_y_reduced))*(nodeedge/nproc_y_full*nproc_y_reduced)+1;
                ...
                for(i1=0;i1<nodeedge/(nproc_x_full/nproc_x_reduced)+1;i1++)
                        for(j1=0;j1<nodeedge/(nproc_y_full/nproc_y_reduced)+1;j1++)
                        {
                                reduced_buffer[index]=M[index_x+i1][index_y+j1];
                                index++;
                        }
/*index=0;
                for(i1=0;i1<nodeedge/(nproc_x_full/nproc_x_reduced)+1;i1++)
                        for(j1=0;j1<nodeedge/(nproc_y_full/nproc_y_reduced)+1;j1++)
                        {
                                printf("%d,%d,%d: %f,%f\n",i,index_x,index_y,reduced_buffer[index++],M[index_x+i1][index_y+j1]);
                                
                        }
*/
                MPI_Send ( &reduced_buffer[0],(nodeedge/(nproc_x_full/nproc_x_reduced)+1)*(nodeedge/(nproc_y_full/nproc_y_reduced)+1) , MPI_DOUBLE, destination+nproc_x_reduced*nproc_y_reduced, 1,MPI_COMM_WORLD);
        }
	/* 
	   Report timing 
	 */ 
	...
	if ( rank == 0 )
	{
		fp = fopen ( "laplace_solution_reduce.txt", "w" );
		...
		fclose ( fp );
		printf ( "Reduced:  Solution written to \"laplace_solution.txt\".\n" );
	}
	/*
	   Terminate MPI.
	 */
	 ...
	 */
{
	...
	if ( rank == 0 ) 
	{
		printf ( "Reduce: did %i iterations\n", it );
